...
const char **get_distro_ascii(const char *distro) {
    if (strstr(distro, "Arch") || strstr(distro, "arch")) return arch_ascii;
    if (strstr(distro, "Debian") || strstr(distro, "debian")) return debian_ascii;
    if (strstr(distro, "Fedora") || strstr(distro, "fedora")) return fedora_ascii;
    if (strstr(distro, "Gentoo") || strstr(distro, "gentoo")) return gentoo_ascii;
    if (strstr(distro, "Ubuntu") || strstr(distro, "ubuntu")) return ubuntu_ascii;
    if (strstr(distro, "Void") || strstr(distro, "void")) return void_ascii;
...
const char *get_distro_color(const char *distro) {
    if (strstr(distro, "Arch") || strstr(distro, "arch")) return ARCH_COLOR;
    if (strstr(distro, "Debian") || strstr(distro, "debian")) return DEBIAN_COLOR;
    if (strstr(distro, "Fedora") || strstr(distro, "fedora")) return FEDORA_COLOR;
    if (strstr(distro, "Gentoo") || strstr(distro, "gentoo")) return GENTOO_COLOR;
    if (strstr(distro, "Ubuntu") || strstr(distro, "ubuntu")) return UBUNTU_COLOR;
    if (strstr(distro, "Void") || strstr(distro, "void")) return VOID_COLOR;
...
void get_distro_name(char *distro) {
    ...
    if (!fp) { strcpy(distro, "Unknown"); return; }

    while (fgets(line, sizeof(line), fp)) {
        if (strncmp(line, "PRETTY_NAME=", 12) == 0) {
            char *value = strchr(line, '=') + 1;
            if (value[0] == '"') { value++; char *end = strchr(value, '"'); if (end) *end = '\0'; }
            strcpy(distro, value);
            fclose(fp);
            return;
        }
...
void get_uptime(char *uptime) {
    struct sysinfo si;
    if (sysinfo(&si) != 0) { strcpy(uptime, "Unknown"); return; }
    ...
    char *ptr = temp;
    if (days > 0) ptr += sprintf(ptr, "%ldd ", days);
    if (hours > 0) ptr += sprintf(ptr, "%ldh ", hours);
...
void get_memory_usage(char *memory) {
    struct sysinfo si;
    if (sysinfo(&si) != 0) { strcpy(memory, "Unknown"); return; }

    unsigned long total = si.totalram * si.mem_unit;
    unsigned long free = si.freeram * si.mem_unit;
    unsigned long used = (total - free);

    sprintf(memory, "%luM / %luM", (unsigned long)(used / (1024*1024)), (unsigned long)(total / (1024*1024)));
}

void get_shell(char *shell) {
    char *shell_path = getenv("SHELL");
    if (!shell_path) { strcpy(shell, "Unknown"); return; }

    char *shell_name = strrchr(shell_path, '/');
    if (shell_name) shell_name++; else shell_name = shell_path;
...
void get_prompt(char *prompt) {
    ...
    pw = getpwuid(getuid());
    if (pw) sprintf(prompt, "%s@%s", pw->pw_name, hostname);
    else strcpy(prompt, "unknown@unknown");
}

void print_usage(const char *program_name) {
    printf("Usage: %s [DISTRO]\n\n", program_name);
    printf("Available distributions:\n  arch, debian, fedora, gentoo, ubuntu, void\n");
}

int main(int argc, char *argv[]) {
    char distro[MAX_LINE_LENGTH], uptime[MAX_LINE_LENGTH], packages[MAX_LINE_LENGTH];
    char memory[MAX_LINE_LENGTH], shell[MAX_LINE_LENGTH], wm[MAX_LINE_LENGTH];
    ...
    for (int i = 0; i < ASCII_ART_LINES; i++) {
        ...
        switch (i) {
            case 0: printf("  %s%s%s", "\033[35m", prompt, RESET); break;
            case 1: printf("  %sos%s      %s", color, RESET, distro); break;
            case 2: printf("  %suptime%s  %s", color, RESET, uptime); break;
            case 3: printf("  %spkgs%s    %s", color, RESET, packages); break;
            case 4: printf("  %sshell%s   %s", color, RESET, shell); break;
            case 5: printf("  %swm/de%s   %s", color, RESET, wm); break;
            case 6: printf("  %smemory%s  %s", color, RESET, memory); break;
        }
        printf("\n");
    }
